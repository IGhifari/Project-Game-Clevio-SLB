...
import imgAstronot from "../../../assets/object/pekerjaan/astronot.png";
...
export default function Level8() {
  ...
  const handleChoose = (card) => {
    if (card.name === "Astronot") {
      setResult("benar");
      // Unlock level 9
      localStorage.setItem(
        "pekerjaanLevelUnlocked",
        JSON.stringify(
          Math.max(9, Number(localStorage.getItem("pekerjaanLevelUnlocked")))
        )
      );
    } else {
      setResult("salah");
